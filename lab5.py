...
def list():
    ...
    if not articles:
        return render_template('/lab5/articles.html',message="У вас нет статей",login=login)
...
def edit(article_id):
    ...
    if request.method == 'GET':
        ...
        if not article:
            db_close(conn, cur)
            return render_template('/lab5/articles.html', message="Статья не найдена", login=login)

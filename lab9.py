...
def final():
    try:
        ...
        if age < 18:
            if gender == 'male':
                message = f"Поздравляю тебя, {name}! Желаю, чтобы этот год был лучше всех прошлых!"
            else:
                message = f"Поздравляю тебя, {name}! Желаю, чтобы в этом году у тебя все получалось!"
        else:
            if gender == 'male':
                message = f"Поздравляю вас, {name}! Желаю исполнения всех желаний!"
            else:
                message = f"Поздравляю вас, {name}! Желаю много мандаринов!"
